...
blob <- function(d, k, r){
	...
	d$clust <- NA
	# start from K roughly equally spaced random locations (just permute a bunch and pick the one with the least smallest distance)
	N <- 100
	mat <- matrix(,N,k)
	mindist <- numeric(N)
	for(n in 1:N){
		i <- sort(sample(1:nrow(d), size=k))
		mat[n,] <- i
		points <- d[i,c('x','y')]
		cb <- combn(3,2)
		NC <- ncol(cb)
		distances <- numeric(NC)
		for(c in 1:NC)distances[c] <- slc(x=points$x[cb[1,c]],y=points$y[cb[1,c]],ax=points$x[cb[2,c]],ay=points$y[cb[2,c]],input='deg')
	...
	# measure its distance to data currently in the clusters (spatially close, temporally distant)
	space.dist <- time.dist <- numeric(k)
	for(n in 1:k){ # loop for each cluster
		clust <- subset(d, clust==n)
		space.dist[n] <- mean(slc(x=p$x,y=p$y,ax=clust$x, ay=clust$y, input='deg'))
		time.dist[n] <- mean(abs(clust$time - p$time))
		}

	# decide which cluster to assign it to. Decision requires a balance between space and time using parameter r
	space.weight <- space.dist/max(space.dist)
	time.weight <- time.dist/max(time.dist)
	stat <- space.weight^r / time.weight
	cluster <- which(stat==min(stat))
	d$clust[p.i] <- cluster
	}
return(d)}
